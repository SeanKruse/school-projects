...
# Definitely the hardest assignment I've worked on. I plan on revisiting after grading to fix the problems
# and better understand bfs
...
# Trying to make print method for bfs
# def traverse(y):
# x = y
# for x in bfs():
# print(x)
...
class TestWaterContainerGraphSearch(unittest.TestCase):
    ...
    def testFindWaterContainerPath(self):
        # Testing different inputs and traversal methods to try to print from bfs
        findWaterContainerPath()
        # print methods from water jugs
        x = 8
        y = 6
        print(findWaterContainerPath.__call__(pour_a_to_b(x, y)))
        print(findWaterContainerPath.__call__(pour_b_to_a(x, y)))
        print(findWaterContainerPath.__call__(pour_out_a(x, y)))
        print(findWaterContainerPath.__call__(pour_out_b(x, y)))
        print(findWaterContainerPath.__call__(fill_a(x, y)))
        print(findWaterContainerPath.__call__(fill_b(x, y)))

        # IMPLEMENT YOUR FIRST TEST HERE

    def test_vertex_single(self):
        graph = {0: []}
        self.assertEqual(bfs(graph, []))
